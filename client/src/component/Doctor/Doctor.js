            ...
            <div className="jumbotron mt-5" style={{ backgroundColor: "#e0e0e0" }}>
              ...
              <div className="list-group-flush" style={{ backgroundColor: "#e0e0e0" }}>
                ...
                {!isLoading ? patients.map(patient => {
                  return (
                    <div key={patient.patient_id} className="list-group-item" style={{ backgroundColor: "#e0e0e0", display: 'flex', flexDirection: 'row' }}>
                      ...
                      <button style={{ marginLeft: '20px' }} onClick={async () => {
                        const value = prompt("Enter Prescription : ");
                        await axios.post("http://localhost:5000/doctor/addPresc", { patientID: patient.patient_id, prec: value }).then((res) => {
                          console.log(res);
                        }).catch((err) => {
